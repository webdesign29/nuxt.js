...
class Renderer {
  ...
  async renderRoute(url, context = {}) {
    ...
    if (this.options.render.csp && this.options.render.csp.enabled) {
      let hash$$1 = crypto.createHash(this.options.render.csp.hashAlgorithm);
      hash$$1.update(serializedSession);
      cspScriptSrcHashes.push(
        `'${this.options.render.csp.hashAlgorithm}-${hash$$1.digest('base64')}'`
      );
    }
...
const warn$1 = msg => console.error(chalk.red(`${prefix} ${msg}\n`)); // eslint-disable-line no-console
const tip$1 = msg => console.log(chalk.yellow(`${prefix} ${msg}\n`)); // eslint-disable-line no-console
